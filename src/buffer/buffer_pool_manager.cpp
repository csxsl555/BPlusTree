...
namespace bustub {
    ...
    auto BufferPoolManager::NewPage(page_id_t *page_id) -> Page * {
        ...
        if (!free_list_.empty()) { //  If I can get a free frame, use the free list first
            frame_to_set = this->free_list_.front();
            free_list_.pop_front();
        } else {
            if (replacer_->Evict(&frame_to_set)) { // Now I have to evict
    ...
    auto BufferPoolManager::FetchPage(page_id_t page_id,
        ...
        std::lock_guard<std::mutex> lock(latch_);
        if (page_table_.find(page_id) != page_table_.end()) { // buffer pool hit! Then I do not need disk IO
        ...
        if (!free_list_.empty()) { // use the free list
